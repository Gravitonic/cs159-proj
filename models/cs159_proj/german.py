...
for size in CORPUS_SIZES:
    ...
    with (
        open(files['de-en']['de'][size], encoding='utf-8', errors='replace') as f,
        open(files['de-en']['en'][size], encoding='utf-8', errors='replace') as e,
    ):
    ...
    for i in range(min(size, len(german_alignments))):
        alignment = german_alignments[i]
